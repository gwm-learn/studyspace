...
进程退出调用 `do_exit`（kernel/exit.c），它释放进程占用的内存页、关闭文件描述符、通知父进程（发送SIGCHLD），最后把进程置为TASK_ZOMBIE并调用schedule。僵死进程的task_struct仍留在task[]中，等父进程 `wait()` 回收。

`wait` 系列系统调用（`sys_waitpid`）在父进程侧运行：若子进程已是僵死状态，就直接回收其task_struct并返回；否则父进程进入不可中断睡眠，直到收到SIGCHLD被唤醒。进程1（init）作为所有孤儿进程的最终父进程，通过不断wait保证没有僵死进程泄漏。

## 信号机制

信号是进程间通信的简单方式，由 `signal.c` 处理。sys_signal注册信号处理函数，sys_kill向目标进程发送信号。信号的递送发生在进程从内核态返回用户态时（`ret_from_sys_call`），通过检查进程的signal位图实现：

1. 进程收到信号，内核在返回用户态前检查signal位图。
2. 按位找出待处理信号，若该信号已注册用户处理函数，则在用户栈上压入处理函数地址。
3. iret后CPU先执行用户信号处理函数，函数返回时再回到被中断的代码。

SIGKILL和SIGSTOP不可被捕获或忽略，其余信号均可由用户注册处理函数。0.11的信号处理是单信号模型的雏形，与后来POSIX的实时信号、信号屏蔽字等机制相比还很简单。
